...
import { X, Send, Zap, Clock, Search, ImagePlus, Loader2, Trash2, Users, MapPin } from "lucide-react";
...
import { createNotificationsBulk, findProsForRequest } from "@/integrations/firebase/notifications";
...
export function RequestModal({ onClose }: RequestModalProps) {
  const t = useT();
  const { userPhone, userCity, userId, userName } = useAppStore();
  const [showAllCats, setShowAllCats] = useState(false);
  const [catSearch, setCatSearch] = useState("");
  const [photos, setPhotos] = useState<string[]>([]);
  const [uploading, setUploading] = useState(false);
  const [audience, setAudience] = useState<"proximity" | "all">("proximity");
  ...
  const onSubmit = async (data: RequestFormData) => {
    try {
      const requestId = await createRequest({ ...data, userId: userId ?? null, photos });
      // Notify matching pros
      try {
        const pros = await findProsForRequest({
          category: data.category,
          city: data.city,
          proximity: audience === "proximity",
        });
        if (pros.length > 0) {
          await createNotificationsBulk(
            pros.map((p) => ({
              userId: p.uid,
              type: "request",
              icon: data.urgency === "urgent" ? "🚨" : "📩",
              title: data.urgency === "urgent" ? "Demande URGENTE" : "Nouvelle demande",
              message: `${data.category} à ${data.city} — ${data.description.slice(0, 80)}${data.description.length > 80 ? "…" : ""}`,
              urgent: data.urgency === "urgent",
              link: "/pro",
              meta: { requestId, fromUserId: userId, fromUserName: userName },
            })),
          );
        }
      } catch (e) {
        console.warn("notify pros failed", e);
      }
      ...
      >
        ...
        {isSubmitSuccessful ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-3">
            ...
            {/* Audience */}
            <div>
              <label className="text-xs font-semibold mb-1.5 block">Envoyer aux artisans</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => setAudience("proximity")}
                  className={`p-2.5 rounded-xl border-2 text-start transition-all ${audience === "proximity" ? "border-primary bg-primary/5" : "border-border bg-card"}`}
                >
                  <div className="flex items-center gap-1.5 font-semibold text-xs"><MapPin size={12} /> À proximité</div>
                  <p className="text-[10px] text-muted-foreground mt-0.5">Même ville</p>
                </button>
                <button
                  type="button"
                  onClick={() => setAudience("all")}
                  className={`p-2.5 rounded-xl border-2 text-start transition-all ${audience === "all" ? "border-primary bg-primary/5" : "border-border bg-card"}`}
                >
                  <div className="flex items-center gap-1.5 font-semibold text-xs"><Users size={12} /> Tous</div>
                  <p className="text-[10px] text-muted-foreground mt-0.5">Du métier</p>
                </button>
              </div>
            </div>
