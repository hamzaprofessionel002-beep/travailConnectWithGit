...
type EntSearch = { category?: string };

export const Route = createFileRoute("/entreprises")({
  component: EntreprisesPage,
  validateSearch: (search: Record<string, unknown>): EntSearch => ({
...
function EntreprisesPage() {
  const navigate = useNavigate({ from: "/entreprises" });
  const { category } = Route.useSearch();
  const selectedCategory = category ?? null;
  const { searchQuery, setSearchQuery } = useAppStore();
  const setSelectedCategory = (next: string | null) =>
    navigate({ search: (prev: EntSearch) => ({ ...prev, category: next ?? undefined }) });
